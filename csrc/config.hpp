...
struct LowLatencyLayout {
    ...
    LowLatencyLayout(void* rdma_buffer, int num_max_dispatch_tokens_per_rank, int hidden, int num_ranks, int num_experts, bool eager_support) {
        ...
        if (eager_support) {
            num_bytes_per_dispatch_msg = EXTEND_FOR_TAG_AND_ALIGN(num_bytes_per_dispatch_msg + sizeof(int), AR_MSG_LONG_ALIGNMENT);
            num_bytes_per_combine_msg = EXTEND_FOR_TAG_AND_ALIGN(num_bytes_per_combine_msg + sizeof(int), AR_MSG_LONG_ALIGNMENT);
        }
