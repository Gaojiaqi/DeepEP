                    ...
                    for use_ue8m0 in (False, True) if round_scale else (False, ):
                        ...
                        for zero_copy in (False, ) if use_logfmt else (False, True):
                            ...
                            combined_x, event, hook = buffer.low_latency_combine(simulated_gemm_x, topk_idx, topk_weights, handle,
                                                                                ...
                                                                                return_recv_hook=return_recv_hook, out=out, eager_opt=eager_opt)
                            hook() if return_recv_hook else event.current_stream_wait()
                            if do_check:
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    global eager_opt
    eager_opt = args.eager_opt
    if local_rank == 0:
        print(f'Eager opt is {eager_opt}', flush=True)

    num_rdma_bytes = deep_ep.Buffer.get_low_latency_rdma_size_hint(num_tokens, hidden, num_ranks, num_experts, True)
    if local_rank == 0:
        print(f'Allocating buffer size: {num_rdma_bytes / 1e6} MB ...', flush=True)
    buffer = deep_ep.Buffer(group, num_rdma_bytes=num_rdma_bytes, low_latency_mode=True,
                            num_qps_per_rank=num_experts // num_ranks,
                            allow_nvlink_for_low_latency_mode=not args.disable_nvlink, explicitly_destroy=True,
                            allow_mnnvl=args.allow_mnnvl, eager_support=True)
    #for _ in range(10):
    #    test_main(num_tokens, hidden, num_experts, num_topk, rank, num_ranks, group, buffer,
    #            use_logfmt=args.use_logfmt, seed=1)

    do_pressure_test = args.pressure_test
    for seed in range(int(os.getenv('TEST_START_SEED', 0)), int(50) if do_pressure_test else 0):
        if local_rank == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eager-opt", type=int, default=3,
                        help='Eager level')
